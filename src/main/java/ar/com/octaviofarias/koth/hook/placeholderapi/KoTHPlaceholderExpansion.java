...
public class KoTHPlaceholderExpansion extends PlaceholderExpansion {
    ...
    public @Nullable String onRequest(OfflinePlayer player, @NotNull String params) {
        if(params.equalsIgnoreCase("active_koths_count")) {
            ...
            if (name.startsWith("[")) {
                int index = Integer.parseInt(name.replace("[", "").replace("]", ""));
                if (index < 0 || index >= KoTHManager.getActiveKoTHs().size()) return "";
                ActiveKoTH ak = KoTHManager.getActiveKoTHs().get(index);
                return ak != null ? KoTHUtils.formatTime(ak.getReamingTime(), DamiXKoTH.getSettings().getSetting("time-format")) : "";
            }
            Optional<ActiveKoTH> ak = KoTHManager.getActiveKoTH(name);
            return ak.isPresent() ? KoTHUtils.formatTime(ak.get().getReamingTime(), DamiXKoTH.getSettings().getSetting("time-format")) : "";
            ...
            if (name.startsWith("[")) {
                ...
                if (index < 0 || index >= KoTHManager.getActiveKoTHs().size()) return "";
                ActiveKoTH ak = KoTHManager.getActiveKoTHs().get(index);
                return ak != null ? KoTHUtils.formatTime(ak.getDominatingTime(), DamiXKoTH.getSettings().getSetting("time-format")) : "";
            }
            Optional<ActiveKoTH> ak = KoTHManager.getActiveKoTH(name);
            return ak.isPresent() ? KoTHUtils.formatTime(ak.get().getDominatingTime(), DamiXKoTH.getSettings().getSetting("time-format")) : "";
        }
            ...
            if (name.startsWith("[")) {
                ...
                ActiveKoTH ak = KoTHManager.getActiveKoTHs().get(index);
                return ak != null
                        ? (ak.getDominating() == null ? DamiXKoTH.getSettings().getSetting("nobody-placeholder") : ak.getDominating().getName())
                        : "";
            }

            Optional<ActiveKoTH> ak = KoTHManager.getActiveKoTH(name);
            return ak.isPresent()
                    ? (ak.get().getDominating() == null ? DamiXKoTH.getSettings().getSetting("nobody-placeholder") : ak.get().getDominating().getName())
                    : "";
        }
